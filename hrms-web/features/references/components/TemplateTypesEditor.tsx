...
export function TemplateTypesEditor() {
  const {
    list,
    sortedList,
    loading,
    ...
    addItem,
    deleteSelected,
  } = useReferenceList<TemplateTypeRow>({
  ...
  const handleAdd = async () => {
    ...
    if (!Number.isInteger(num) || num < 0 || !name) {
      toast.error("Укажите целое число (номер) и имя типа");
      return;
    }
    ...
    try {
      ...
      setNewName("");
      setAddOpen(false);
      toast.success("Тип шаблона добавлен");
    } catch (e) {
    ...
    <div className="flex flex-1 flex-col gap-4 p-4">
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="min-w-[200px]">Имя типа шаблона</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {sortedList.map((row) => (
      ...
      {sortedList.length === 0 && (
        <p className="text-center text-sm text-muted-foreground">
          Нет записей. Нажмите «Добавить», чтобы создать тип шаблона.
        </p>
      )}
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="sm:max-w-md" showCloseButton>
          <DialogHeader>
            <DialogTitle>Добавить тип шаблона</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                placeholder="Напр. Шапка сводного приказа"
                onKeyDown={(e) => e.key === "Enter" && handleAdd()}
              />
            </div>
          </div>
